/* Running program on Raspberry Pi 3B+
XBee module adapter is connected on USB port
Open points:
- You need to launch minicom before, otherwise uart is not properly inizialized
*/
...
while(run) {
	...
	switch(status) {
		...
		case UART_RECEIVING:
			if ( (count = read(uart, &c,1)) < 0)//Read from uart
					if ( (errno != EAGAIN) & (errno!=EWOULDBLOCK) ) {
						perror("Read error in UART_RECEIVING.");
						status = END_PROGRAM;
					break;
					}
		...
		case DISCARD_TELEGRAM:
			printf("Uart telegram discarded.\n");
			status = IDLE;
			break;
